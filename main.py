...
def get_config():
    ...
    config = {
        ...
        "twitter": {
            "consumer_key": "myJRsoneDPkX6WRIOneVq8Xun",
            "consumer_sec":
            "YpJU2DT8jxMzcMXalpB8uc9GVT81Wock3AOWpqo8MLZNJ7rBwR",
            "access_key": "",
            "access_sec": ""
        }
    }
    if not path.exists(config_path):
        logger.debug(
            "{} not found, making default config file.".format(config_path))
        # デフォルトの設定を作成する
        if not path.exists(CONFIG_DIR):
            makedirs(
                CONFIG_DIR,
                exist_ok=True)  # make ~/.cache/, ~/.cache/buttweet/
...
def get_playlog(player_id, plus):
    ...
    for i in history[1:]:
        ...
        if len(row) == 5 and row[4] != "":
            ...
            daily += [
                playtime, int(row[4].split()[0].replace(',', '')),
...
def daily_tweet(playlog, auth, config):
    ...
    # 最後に投稿した、latest_play_dateの値
    last_tweeted_date_str = worklog.get("last_daily_tweet_date")
    last_tweeted_date = datetime.datetime.strptime(
        last_tweeted_date_str, '%Y-%m-%d'
    ).date() if last_tweeted_date_str else datetime.date.min
    ...
    if result:
        worklog["last_daily_tweet_url"] = "https://twitter.com/{status.user.screen_name}/status/{status.id_str}".format(status=result)
        set_worklog(worklog)
...
@click.option(
    "--force", is_flag=True, help="前回実行時の結果に関わらず投稿する。--daemonとの併用は不可能。")
@click.option(
    "--daemon",
    "-d",
    is_flag=True,
...
@click.option(
    "--dry-run", is_flag=True, help="このオプションを指定した場合、投稿をせずに標準出力への出力を行う")
@click.option("--verbose", "-v", count=True, help="ログレベルを指定する。")
def main(player_id, plus, summary, twitter_auth, force, daemon, dry_run,
         verbose):
    ...
    if daemon and force:
        raise ValueError(
            "Both daemon and force was specified; it will cause infinite tweeting."
        )
    ...
    if twitter_auth:
        access_key, access_sec = twitter_authorize(twitter["consumer_key"],
                                                   twitter["consumer_sec"])
    ...
    if not player_id:
        raise ValueError(
            "Player ID not set, use config or argument(see --help)")
    else:
